...
import { Activity, Eye, X, Clock, Calendar, Heart, Thermometer, Droplet, AlertCircle, CheckCircle, FileText, Stethoscope } from 'lucide-react';
...
export const PatientHealthHistory = ({ healthLogs, showAll = false }) => {
  ...
  // Get doctor name from recordedBy
  ...
  const getStatusBadge = (status) => {
    if (status === 'critical') {
      return <span className="badge critical"><AlertCircle size={12} /> Critical</span>;
    }
    if (status === 'warning') {
      return <span className="badge warning"><AlertCircle size={12} /> Warning</span>;
    }
          ...
          <tbody>
            {displayedLogs.map(log => (
              <React.Fragment key={log._id}>
                <tr className={`log-row ${log.status || 'normal'}`}>
          ...
          <div className="record-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              <div className="detail-row">
                ...
                <span className="doctor-name">{getDoctorName(selectedRecord.recordedBy)}</span>
              </div>
              <div className="detail-row">
                <Heart size={16} />
              ...
              <div className="vitals-section">
                ...
                <div className="vitals-grid">
                  <div className="vital-item">
                    ...
                    <span className="vital-value">{selectedRecord.vitals?.heartRate || '--'} <span className="unit">bpm</span></span>
                  </div>
                  <div className="vital-item">
                    <span className="vital-name">Blood Pressure:</span>
                    <span className="vital-value">{selectedRecord.vitals?.bloodPressure?.systolic || '--'}/{selectedRecord.vitals?.bloodPressure?.diastolic || '--'} <span className="unit">mmHg</span></span>
                  </div>
                  <div className="vital-item">
                    <span className="vital-name">Temperature:</span>
                    <span className="vital-value">{selectedRecord.vitals?.temperature || '--'} <span className="unit">°C</span></span>
                  </div>
                  <div className="vital-item">
                    <span className="vital-name">O₂ Saturation:</span>
                    <span className="vital-value">{selectedRecord.vitals?.oxygenSaturation || '--'} <span className="unit">%</span></span>
                  </div>
                  {selectedRecord.vitals?.respiratoryRate && (
                    <div className="vital-item">
                      <span className="vital-name">Respiratory Rate:</span>
                      <span className="vital-value">{selectedRecord.vitals.respiratoryRate} <span className="unit">/min</span></span>
                    </div>
                  )}
                </div>
              </div>
