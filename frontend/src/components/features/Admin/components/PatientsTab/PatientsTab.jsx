...
import { Plus, Edit, Trash2, Users, Mail, Phone, Stethoscope, CheckCircle, AlertCircle } from 'lucide-react';
      ...
      <div className="table-wrapper">
        {filteredPatients.length === 0 ? (
          ...
          <table className="data-table">
            <thead><tr><th>Patient</th><th>Contact</th><th>Assigned Doctor</th><th>Status</th><th>Actions</th></tr></thead>
            <tbody>
                ...
                <tr key={patient._id} className={!patient.user?.isActive ? 'inactive-row' : ''}>
                  <td>
                    <div className="patient-cell">
                      ...
                      <div><div className="patient-name">{patient.user?.profile?.firstName} {patient.user?.profile?.lastName}</div><div className="patient-email"><Mail size={12} /> {patient.user?.email}</div></div>
                    </div>
                  </td>
                  ...
                  <td>
                    <div className="action-buttons">
                      <button className={`action-icon status ${patient.user?.isActive === true ? 'deactivate' : 'activate'}`} onClick={() => onToggleStatus(patient._id, patient.user?.isActive === true, `${patient.user?.profile?.firstName} ${patient.user?.profile?.lastName}`)}>{patient.user?.isActive === true ? '🔴' : '🟢'}</button>
                      <button className="action-icon edit" onClick={() => onEdit(patient)}><Edit size={14} /></button>
                      <button className="action-icon delete" onClick={() => onDelete(patient._id, `${patient.user?.profile?.firstName} ${patient.user?.profile?.lastName}`)}><Trash2 size={14} /></button>
                    </div>
                  </td>
                </tr>
              ))}
