...
const PatientsTab = ({ patients, doctors, onAdd, onEdit, onDelete, onToggleStatus }) => {
  ...
  const handleDeleteWithConfirm = (patientId, patientName) => {
    showConfirm(
      'Delete Patient',
  ...
  const handleToggleStatusWithConfirm = (patientId, isActive, patientName) => {
    ...
    showConfirm(
      ...
      { id: patientId, isActive, name: patientName }
    );
  };
        ...
        <div className="table-wrapper">
          {filteredPatients.length === 0 ? (
            ...
            <table className="data-table">
              <thead>
                <tr><th>Patient</th><th>Contact</th><th>Assigned Doctor</th><th>Status</th><th>Actions</th></tr>
              </thead>
              <tbody>
                {filteredPatients.map(patient => (
                  <tr key={patient._id} className={!patient.user?.isActive ? 'inactive-row' : ''}>
                    ...
                    <td><div className="contact-phone"><Phone size={12} /> {patient.user?.profile?.phone || 'No phone'}</div></td>
                    <td>{patient.assignedDoctor?.profile?.firstName ? <div className="assigned-doctor"><Stethoscope size={12} /> Dr. {patient.assignedDoctor.profile.firstName} {patient.assignedDoctor.profile.lastName}</div> : <span className="unassigned-badge">Not assigned</span>}</td>
                    <td><div className={`status-badge ${patient.user?.isActive === true ? 'active' : 'inactive'}`}><span className="status-dot"></span>{patient.user?.isActive === true ? 'Active' : 'Inactive'}</div></td>
                    <td>
                      <div className="action-buttons">
                        <button className="action-icon view" onClick={() => handleViewEHR(patient)} title="View Electronic Health Record"><Eye size={14} /></button>
                        <button className={`action-icon status ${patient.user?.isActive === true ? 'deactivate' : 'activate'}`} onClick={() => handleToggleStatusWithConfirm(patient._id, patient.user?.isActive === true, `${patient.user?.profile?.firstName} ${patient.user?.profile?.lastName}`)} title={patient.user?.isActive === true ? 'Deactivate' : 'Activate'}>{patient.user?.isActive === true ? '🔴' : '🟢'}</button>
                        <button className="action-icon edit" onClick={() => onEdit(patient)} title="Edit Patient"><Edit size={14} /></button>
                        <button className="action-icon delete" onClick={() => handleDeleteWithConfirm(patient._id, `${patient.user?.profile?.firstName} ${patient.user?.profile?.lastName}`)} title="Delete Patient"><Trash2 size={14} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
