    ...
    <div className="admin-dashboard">
      ...
      <main className="dashboard-main">
        ...
        <div className="admin-tabs">
          ...
          <button className={`tab-btn ${activeTab === 'doctors' ? 'active' : ''}`} onClick={() => setActiveTab('doctors')}>Doctors</button>
          <button className={`tab-btn ${activeTab === 'patients' ? 'active' : ''}`} onClick={() => setActiveTab('patients')}>Patients</button>
          <button className={`tab-btn ${activeTab === 'specializations' ? 'active' : ''}`} onClick={() => setActiveTab('specializations')}>Specializations</button>
        </div>
        ...
        {activeTab === 'patients' && (
          <div className="patients-tab">
            <div className="tab-header">
              ...
              }}><Plus size={16} /> Add Patient</Button>
            </div>
        ...
        {activeTab === 'specializations' && (
          <div className="specializations-tab">
            <div className="tab-header">
              ...
              }}><Plus size={16} /> Add Specialization</Button>
            </div>
                  ...
                  <div key={spec._id} className="specialization-card">
                    <div className="specialization-header">
                      ...
                      <div className="specialization-actions">
                        ...
                        <button className="delete-btn" onClick={() => handleDeleteSpecialization(spec._id, spec.name)}><Trash2 size={16} /></button>
                      </div>
                    </div>
      ...
      {showEditDoctorModal && editingDoctor && (
        <div className="modal-overlay" onClick={() => setShowEditDoctorModal(false)}>
          <div className="modal-content doctor-modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header"><h3>Edit Doctor: Dr. {editingDoctor.profile?.firstName} {editingDoctor.profile?.lastName}</h3><button className="close-btn" onClick={() => setShowEditDoctorModal(false)}>×</button></div>
            <form onSubmit={handleUpdateDoctor}>
              <div className="form-row"><input type="text" placeholder="First Name" value={editDoctorForm.firstName} onChange={(e) => setEditDoctorForm({...editDoctorForm, firstName: e.target.value})} required /><input type="text" placeholder="Last Name" value={editDoctorForm.lastName} onChange={(e) => setEditDoctorForm({...editDoctorForm, lastName: e.target.value})} required /></div>
      ...
      {showPatientModal && (
        <div className="modal-overlay" onClick={() => setShowPatientModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header"><h3>{editingPatient ? 'Edit Patient' : 'Add New Patient'}</h3><button className="close-btn" onClick={() => setShowPatientModal(false)}>×</button></div>
            <form onSubmit={handleSavePatient}>
              <div className="form-row"><input type="text" placeholder="First Name" value={patientForm.firstName} onChange={(e) => setPatientForm({...patientForm, firstName: e.target.value})} required /><input type="text" placeholder="Last Name" value={patientForm.lastName} onChange={(e) => setPatientForm({...patientForm, lastName: e.target.value})} required /></div>
              ...
              <select value={patientForm.bloodType} onChange={(e) => setPatientForm({...patientForm, bloodType: e.target.value})}><option value="">Select Blood Type</option><option value="A+">A+</option><option value="A-">A-</option><option value="B+">B+</option><option value="B-">B-</option><option value="O+">O+</option><option value="O-">O-</option><option value="AB+">AB+</option><option value="AB-">AB-</option></select>
              ...
              <select value={patientForm.assignedDoctor} onChange={(e) => setPatientForm({...patientForm, assignedDoctor: e.target.value})}><option value="">Assign Doctor</option>{doctors.map(doc => <option key={doc._id} value={doc._id}>Dr. {doc.profile?.firstName} {doc.profile?.lastName} - {doc.specialization}</option>)}</select>
              <div className="info-note"><AlertCircle size={14} /> Patient will receive temporary password: patient123</div>
              <div className="modal-actions"><button type="button" onClick={() => setShowPatientModal(false)}>Cancel</button><button type="submit" disabled={loadingSubmit}>{loadingSubmit ? 'Saving...' : (editingPatient ? 'Update' : 'Create')}</button></div>
            </form>
          </div>
        </div>
      )}
      ...
      {showSpecializationModal && (
        <div className="modal-overlay" onClick={() => setShowSpecializationModal(false)}>
          <div className="modal-content small" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header"><h3>{editingSpecialization ? 'Edit Specialization' : 'Add New Specialization'}</h3><button className="close-btn" onClick={() => setShowSpecializationModal(false)}>×</button></div>
            <form onSubmit={handleSaveSpecialization}>
              ...
              <div className="modal-actions"><button type="button" onClick={() => setShowSpecializationModal(false)}>Cancel</button><button type="submit" disabled={loadingSubmit}>{loadingSubmit ? 'Saving...' : (editingSpecialization ? 'Update' : 'Add')}</button></div>
            </form>
          </div>
        </div>
      )}
